...
1. **Restauración y conexión a la base de datos:** Asegúrate de tener restaurada la base de datos de muestra AdventureWorksLT. Conéctate a tu instancia local de SQL Server, abre una "Nueva Consulta" (New Query) y selecciona la base de datos AdventureWorksLT en el selector superior o ejecuta `USE AdventureWorksLT;`.
2. **Verificación de tablas maestras:** Ejecuta las consultas de verificación sobre `SalesLT.Customer`, `SalesLT.SalesOrderHeader` y `SalesLT.Product` para confirmar el correcto acceso a los datos de prueba.
3. **Implementación de objetos de programabilidad:**
